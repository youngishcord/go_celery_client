...
Используется только библиотека для связи с Rabbit `amqp091-go`, остальное реализовано с нуля.
...
- [ ] Ответ на сообщения в Rabbit через RPC.
...
По всей видимости Rabbit сам определяет получателя сообщения по принципу roundrobin. Но я точно
...
В стандарте у каждой базовой задачи, получаемой от Rabbit при использовании
...
#### Ограничения на количество сообщений в Rabbit
...
В Celery предусметрено два варианта ограничения по времени, `soft_time_limit` и `time_limit`. Мягкое ограничение вызывает ошибку при выполнении операции и ждет корректного завершения процесса, я жесткое ограничение пытается убить процесс принудительно. Как это используется на практике:

- Идет выполнение функции, у которой установлено ограничение по времени выполнения операции.
- Время просрочено. В задаче вызывается исключение и программа пытается остановить операцию и вернуть ошибку.
- Если ошибка возвращается до жесткого таймаута, все хорошо.
- Если программа не успела, начинается процесс принудительной остановки выполнения функции.

В случае с Golang мне скорее всего нужен контекст, при этом, основной контекст создается от жесткого времени, от него создается контекст на мягкое время, который и передается в остальные функции.

## Очереди и роутинг задач

### Декларация очередей и роутеров в Rabbit

Как оказалось, в RabbitMQ мало просто декларировать очередь сообщений, ее необходимо привязать ее к какому то из exchange каналов, в том случае, если есть необходимость использовать отличный от имени очереди routing_key. В противном случае можно использовать дефолтный exchange, который RabbitMQ создает автоматически, и автоматически привязывает к очереди, при ее инициализации.
